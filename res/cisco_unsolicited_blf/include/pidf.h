 ...
 * given (exten_state, presence_state) pair. The state-change dedup gate
 * in res_pjsip_cisco_unsolicited_blf.c compares this against the bits it
 * last sent for a watched extension to skip a NOTIFY whose wire body
 * would be identical — Asterisk fires the extension_state callback for
 * transitions that don't change what BLF watchers see (e.g. INUSE →
 * INUSE|RINGING when alerting is suppressed by the engaged-line rule).
 *
